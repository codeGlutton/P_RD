...
namespace ShopFullGeneratedWidgetBuilder
{
	...
	constexpr TCHAR ArtPackagePath[] = TEXT("/Game/UI/Shop/Art");
	...
	FString SharedBackgroundSourceFile()
	{
		return FPaths::Combine(FPaths::ProjectDir(), TEXT("SourceArt"),
			TEXT("UI"), TEXT("ShopFullGenerated"),
			FString(SharedBackgroundName) + TEXT(".png"));
	}
	...
	{
		const FString SourceFile = SharedBackgroundSourceFile();
		checkf(FPaths::FileExists(SourceFile),
			TEXT("Missing selected KayKit shop background: %s"), *SourceFile);
		...
		checkf(Background != nullptr,
			TEXT("Failed to import selected KayKit shop background: %s"), *ObjectPath);
		Background->Modify();
		Background->LODGroup = TEXTUREGROUP_UI;
		Background->MipGenSettings = TMGS_NoMipmaps;
		Background->CompressionSettings = TC_EditorIcon;
		Background->NeverStream = true;
		Background->VirtualTextureStreaming = false;
		Background->SRGB = true;
		Background->AddressX = TA_Clamp;
		Background->AddressY = TA_Clamp;
		Background->PostEditChange();
	...
	void Build()
	{
		...
		UTexture2D* SkillSlotNormal = Texture(TEXT("/Game/UI/Shop/T_ShopFG_SkillSlot07_Square.T_ShopFG_SkillSlot07_Square"));
		UTexture2D* SkillSlotSelected = Texture(TEXT("/Game/UI/Shop/T_ShopFG_SkillSlot07_Square_Selected.T_ShopFG_SkillSlot07_Square_Selected"));
		UTexture2D* RestUnitPanel = Texture(TEXT("/Game/UI/Shop/T_ShopFG_RestUnitPanel_Clean.T_ShopFG_RestUnitPanel_Clean"));
