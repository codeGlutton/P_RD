...
namespace ShopWidgetBuilder
{
	constexpr TCHAR PackagePath[] = TEXT("/Game/UI/Shop");
	constexpr TCHAR AssetName[] = TEXT("WBP_Shop");
	constexpr TCHAR AssetPath[] = TEXT("/Game/UI/Shop/WBP_Shop.WBP_Shop");
	...
	UWidgetBlueprint* EnsureBlueprint()
	{
		...
		Factory->ParentClass = UShopUIWidgetBase::StaticClass();
	...
	void ResetTree(UWidgetBlueprint* Blueprint)
	{
		...
		Blueprint->ParentClass = UShopUIWidgetBase::StaticClass();
	}

	void Build()
	{
		UE_LOG(LogTemp, Display, TEXT("RD_SHOP_BUILD begin"));

		// Resolve all hard dependencies before replacing the existing tree. A missing
		// generated background therefore cannot leave WBP_Shop half rebuilt.
		UTexture2D* ArtifactBackground = Texture(TEXT(
			"/Game/SVN/OutSideAsset/AICreation/UI/Shop/Backgrounds/"
			"T_Shop_ArtifactMarket_Background.T_Shop_ArtifactMarket_Background"));
		UTexture2D* SkillBackground = Texture(TEXT(
			"/Game/SVN/OutSideAsset/AICreation/UI/Shop/Backgrounds/"
			"T_Shop_SkillCourtyard_Background.T_Shop_SkillCourtyard_Background"));
		UTexture2D* RestBackground = Texture(TEXT(
			"/Game/SVN/OutSideAsset/AICreation/UI/Shop/Backgrounds/"
			"T_Shop_RestInn_Background.T_Shop_RestInn_Background"));
		UTexture2D* TitlePlate = Texture(TEXT(
			"/Game/SVN/OutSideAsset/AICreation/UI/Marchbound/Hire/"
			"T_MB_HireTitlePlate.T_MB_HireTitlePlate"));
		UTexture2D* BackPlate = Texture(TEXT(
			"/Game/SVN/OutSideAsset/AICreation/UI/Marchbound/Hire/"
			"T_MB_HireBackButton.T_MB_HireBackButton"));
		UTexture2D* BuyPlate = Texture(TEXT(
			"/Game/SVN/OutSideAsset/AICreation/UI/Marchbound/Hire/"
			"T_MB_HireDepartButton.T_MB_HireDepartButton"));
		UTexture2D* CellNormal = Texture(TEXT(
			"/Game/SVN/OutSideAsset/AICreation/UI/Marchbound/Hire/"
			"T_MB_HireRowNormal.T_MB_HireRowNormal"));
		UTexture2D* CellSelected = Texture(TEXT(
			"/Game/SVN/OutSideAsset/AICreation/UI/Marchbound/Hire/"
			"T_MB_HireRowSelected.T_MB_HireRowSelected"));
		UTexture2D* RailCardNormal = Texture(TEXT(
			"/Game/SVN/OutSideAsset/AICreation/UI/Shop/Panels/"
			"T_Shop_RailCard_Normal.T_Shop_RailCard_Normal"));
		UTexture2D* RailCardSelected = Texture(TEXT(
			"/Game/SVN/OutSideAsset/AICreation/UI/Shop/Panels/"
			"T_Shop_RailCard_Selected.T_Shop_RailCard_Selected"));
		UTexture2D* SmallButton = Texture(TEXT(
			"/Game/SVN/OutSideAsset/AICreation/UI/Marchbound/Defeat/"
			"T_MB_Defeat_ButtonSecondary.T_MB_Defeat_ButtonSecondary"));
		UTexture2D* WideButton = Texture(TEXT(
			"/Game/SVN/OutSideAsset/AICreation/UI/Marchbound/Defeat/"
			"T_MB_Defeat_ButtonPrimary.T_MB_Defeat_ButtonPrimary"));
		UTexture2D* GoldIcon = Texture(TEXT(
			"/Game/SVN/OutSideAsset/AICreation/UI/Concept02/"
			"T_gold_icon.T_gold_icon"));
		UTexture2D* ArtifactIcons[5] = {
			...
			Texture(TEXT("/Game/SVN/OutSideAsset/AICreation/UI/Artifacts/T_Artifact_TravelersMap.T_Artifact_TravelersMap")),
			Texture(TEXT("/Game/SVN/OutSideAsset/AICreation/UI/Artifacts/T_Artifact_ThornCrest.T_Artifact_ThornCrest"))
		};
		UTexture2D* UnitIcons[3] = {
		...
		UTexture2D* SkillIcons[4] = {
			Texture(TEXT("/Game/SVN/OutSideAsset/AICreation/UI/CombatHUD/SkillIcons/T_SkillIcon_Barrier.T_SkillIcon_Barrier")),
			Texture(TEXT("/Game/SVN/OutSideAsset/AICreation/UI/CombatHUD/SkillIcons/T_SkillIcon_Slash.T_SkillIcon_Slash")),
			Texture(TEXT("/Game/SVN/OutSideAsset/AICreation/UI/CombatHUD/SkillIcons/T_SkillIcon_Whirlwind.T_SkillIcon_Whirlwind")),
			Texture(TEXT("/Game/SVN/OutSideAsset/AICreation/UI/CombatHUD/SkillIcons/T_SkillIcon_HeavySmash.T_SkillIcon_HeavySmash"))
		};

		UWidgetBlueprint* Blueprint = EnsureBlueprint();
		if (Blueprint == nullptr || Blueprint->WidgetTree == nullptr)
		{
			UE_LOG(LogTemp, Error, TEXT("RD_SHOP_BUILD could not create %s"), AssetPath);
			return;
		}
		ResetTree(Blueprint);
		...
		AddLabeledButton(Blueprint, Screen, TEXT("ArtifactTabHolder"),
			...
			WideButton, NSLOCTEXT("ShopHorizontalRail", "Artifact", "아티팩트"),
		...
		AddLabeledButton(Blueprint, Screen, TEXT("SkillTabHolder"),
			...
			WideButton, NSLOCTEXT("ShopHorizontalRail", "Skill", "스킬"),
		...
		AddLabeledButton(Blueprint, Screen, TEXT("RestTabHolder"),
			...
			WideButton, NSLOCTEXT("ShopHorizontalRail", "Rest", "휴식"),
			FVector2D(898.f, 21.f), FVector2D(242.f, 69.f), 12, 27);

		AddImage(Blueprint, Screen, TEXT("GoldPlate"), CellNormal,
			FVector2D(1290.f, 24.f), FVector2D(286.f, 64.f), 10, true);
		AddImage(Blueprint, Screen, TEXT("GoldIcon"), GoldIcon,
			FVector2D(1312.f, 32.f), FVector2D(48.f, 48.f), 12);
		AddText(Blueprint, Screen, TEXT("mGoldText"), FText::FromString(TEXT("0 G")), 26,
			FVector2D(1368.f, 28.f), FVector2D(182.f, 48.f), 12);
		AddLabeledButton(Blueprint, Screen, TEXT("InventoryHolder"),
			TEXT("InventoryPlate"), TEXT("mInventoryButtonText"), TEXT("mInventoryButton"),
			WideButton, NSLOCTEXT("ShopHorizontalRail", "Inventory", "인벤토리"),
		...
		for (int32 Index = 0; Index < 3; ++Index)
		{
			...
			AddImage(Blueprint, SkillPanel,
				...
				Index == 0 ? CellSelected : CellNormal, Position, Extent, 8, true);
			AddImage(Blueprint, SkillPanel,
				FName(*FString::Printf(TEXT("mUnitSelectIcon_%d"), Index)), UnitIcons[Index],
		...
		for (int32 Index = 0; Index < 3; ++Index)
		{
			...
			AddImage(Blueprint, RowHolder,
				FName(*FString::Printf(TEXT("RestUnitIcon_%d"), Index)), UnitIcons[Index],
			...
			AddSolidImage(Blueprint, RowHolder,
				...
				FVector2D(450.f, RowY + 64.f), FVector2D(310.f, 24.f), 8);
			AddSolidImage(Blueprint, RowHolder,
				FName(*FString::Printf(TEXT("RestUnitHPBeforeFill_%d"), Index)),
				FLinearColor(.68f, .11f, .13f, 1.f),
				FVector2D(454.f, RowY + 68.f),
			...
			AddSolidImage(Blueprint, RowHolder,
				...
				FLinearColor(.025f, .04f, .055f, .96f),
				FVector2D(846.f, RowY + 64.f), FVector2D(420.f, 24.f), 8);
			AddSolidImage(Blueprint, RowHolder,
				FName(*FString::Printf(TEXT("RestUnitHPAfterFill_%d"), Index)),
				FLinearColor(.78f, .15f, .17f, 1.f),
				FVector2D(850.f, RowY + 68.f), FVector2D(412.f, 16.f), 9);
			AddText(Blueprint, RowHolder,
		...
		AddText(Blueprint, RestPanel, TEXT("RestCostText"), FText::FromString(TEXT("100 G")), 24,
			FVector2D(804.f, 757.f), FVector2D(150.f, 48.f), 14,
		...
		AddLabeledButton(Blueprint, RestPanel, TEXT("RestButtonHolder"),
			...
			BuyPlate, NSLOCTEXT("ShopHorizontalRail", "RestCTA", "휴식하기"),
		...
		for (int32 Index = 0; Index < 5; ++Index)
		{
			...
			UImage* Icon = AddImage(Blueprint, Screen,
				FName(*FString::Printf(TEXT("ShopRailIcon_%d"), Index)), ArtifactIcons[Index],
		...
		AddImage(Blueprint, Screen, TEXT("mSelectedItemIcon"), ArtifactIcons[2],
		...
		AddLabeledButton(Blueprint, Screen, TEXT("PreviousHolder"),
			...
			SmallButton, FText::FromString(TEXT("<")),
		...
		for (int32 Index = 0; Index < 4; ++Index)
		{
			...
			AddImage(Blueprint, SkillPanel,
				...
				Index == 2 ? CellSelected : CellNormal, Position, Extent, 35, true);
			AddImage(Blueprint, SkillPanel,
				FName(*FString::Printf(TEXT("mSkillSlotIcon_%d"), Index)), SkillIcons[Index],
		...
		AddImage(Blueprint, ArtifactInventoryPanel, TEXT("ArtifactInventoryFrame"),
			CellNormal, FVector2D(245.f, 145.f), FVector2D(1110.f, 700.f), 2, true,
		...
		AddLabeledButton(Blueprint, ArtifactInventoryPanel,
			...
			WideButton, NSLOCTEXT("ShopHorizontalRail", "CloseInventory", "닫기"),
		...
		if (!UPackage::SavePackage(Blueprint->GetPackage(), Blueprint, *Filename,
			FSavePackageArgs()))
		{
			UE_LOG(LogTemp, Error, TEXT("RD_SHOP_BUILD save failed"));
			return;
		}

		UE_LOG(LogTemp, Display,
			TEXT("RD_SHOP_BUILD success asset=%s design=1600x1000 rail_slots=5 unit_slots=3 skill_slots=4"),
			AssetPath);
	}
}

void RegisterShopWidgetBuilderCommands()
{
	using namespace ShopWidgetBuilder;
	BuildCommand = MakeUnique<FAutoConsoleCommand>(
		TEXT("RD.Editor.BuildShop"),
		TEXT("Rebuild WBP_Shop with the horizontal-rail shop layout."),
		FConsoleCommandDelegate::CreateStatic(&Build));
}

void UnregisterShopWidgetBuilderCommands()
{
	ShopWidgetBuilder::BuildCommand.Reset();
}
