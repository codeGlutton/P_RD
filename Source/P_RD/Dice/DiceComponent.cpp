...
/** @brief 주사위 상태만 들고 있는 컴포넌트라 Tick을 끈다. */
UDiceComponent::UDiceComponent()
{
	// 데이터 컴포넌트라 매 프레임 틱이 필요 없다.
	PrimaryComponentTick.bCanEverTick = false;
}

/** @brief 런 보유 DiceId 목록을 UDiceModel 런타임 인스턴스로 변환한다. */
void UDiceComponent::BuildFromDiceIds(const TArray<FPrimaryAssetId>& DiceIds)
{
	mDice.Reset();
...
void UDiceComponent::RollAll(const FRandomStream& Stream)
{
...
void UDiceComponent::MarkDiceUsed(int32 DiceIndex)
{
...
int32 UDiceComponent::GetRolledDiceValue(int32 DiceIndex) const
{
