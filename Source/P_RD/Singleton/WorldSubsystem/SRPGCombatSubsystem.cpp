...
void USRPGCombatSubsystem::UnregisterTurns(const AUnit* Owner)
{
...
bool USRPGCombatSubsystem::UnregisterTurnImmediately(TSharedRef<FSRPGTurnContext> TurnContext)
{
...
void USRPGCombatSubsystem::FlushPendingTurnRequests()
{
...
void USRPGCombatSubsystem::RegisterEnemyUnit(const FEnemyUnitPlacementData& EnemyPlacementData)
{
...
void USRPGCombatSubsystem::RegisterObstacle(const FObstaclePlacementData& ObstaclePlacementData)
{
	checkf(mTileMap != nullptr, TEXT("타일맵 미존재"));
...
void USRPGCombatSubsystem::SpawnTileMap()
{
...
void USRPGCombatSubsystem::RegisterEnemyUnits(const TArray<FEnemyUnitPlacementData>& EnemyPlacementDatas)
{
	checkf(mTileMap != nullptr, TEXT("타일맵 미존재"));
...
void USRPGCombatSubsystem::RegisterObstacles(const TArray<FObstaclePlacementData>& ObstaclePlacementDatas)
{
	checkf(mTileMap != nullptr, TEXT("타일맵 미존재"));
...
UCombatViewModel* USRPGCombatSubsystem::GetCombatViewModel()
{
	if (mCombatViewModel == nullptr)
	{
		mCombatViewModel = NewObject<UCombatViewModel>(this);
	}
	return mCombatViewModel;
}
