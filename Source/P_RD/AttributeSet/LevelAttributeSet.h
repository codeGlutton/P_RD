...
public:
	static int32 GetMaxLevel(const UObject* WorldContextObject);

	/*
	 * @brief 레벨별 최대 경험치 배열을 추출합니다.
	 * @param WorldContextObject 월드 컨텍스트 객체
	 * @return 레벨 순서대로 나열된 경험치 배열
	 */
	static TArray<float> GetMaxExps(const UObject* WorldContextObject);
	static bool GetMaxExps(const UObject* WorldContextObject, OUT TArray<float>& OutMaxExps);

	/*
	 * @brief 특정 레벨의 최대 경험치를 추출합니다.
	 * @param WorldContextObject 월드 컨텍스트 객체
	 * @param Level 대상 레벨
	 * @return 해당 레벨의 최대 경험치
	 */
	static float GetMaxExp(const UObject* WorldContextObject, int32 Level);
	static bool GetMaxExp(const UObject* WorldContextObject, int32 Level, OUT float& OutMaxExp);
	...
	static bool GetRarityRates(const UObject* WorldContextObject, OUT TArray<FRarityRate>& OutRates);
	...
	static bool GetRarityRate(const UObject* WorldContextObject, int32 Level, OUT FRarityRate& OutRate);
	...
	static bool GetPrices(const UObject* WorldContextObject, OUT TArray<float>& OutPrices);
	...
	static bool GetPrice(const UObject* WorldContextObject, int32 Level, OUT float& OutPrice);

public:
	TACTICAL_ATTRIBUTE_ACCESSORS_BASIC(ULevelAttributeSet, MaxExp)
