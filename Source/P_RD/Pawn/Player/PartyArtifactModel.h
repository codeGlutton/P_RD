/*****************************************************************//**
 * @file   PartyArtifactModel.h
 * @brief  파티 보유 아티펙트 원본/배포 모델 정의 헤더
 * @author 이문환
 * @date   2026-07-23
 *********************************************************************/
...
#include "RDMinimal.h"
#include "PartyArtifactModel.generated.h"
 ...
 * - 룸 수명이 아닌 런 수명 객체로, 시뮬레이션 룸 복제 대상 아니므로 UObjectModel 대신 UObject 파생으로 구현
 */
UCLASS()
class P_RD_API UPartyArtifactModel : public UObject
{
	GENERATED_BODY()

	// 유닛테스트용 friend 선언
	friend class FPartyArtifactModelTests;

public:
	/**
	 * @brief 아티펙트 획득 (파티 목록에 추가하고 파티 구성원 전원에 장착)
	 * @param Data 획득할 아티펙트 데이터 (로드 완료된 포인터)
	 * @param PartyMembers 파티 구성원 목록
	 * @return 성공 여부 (Data가 nullptr이면 false)
	 */
	bool AddArtifact(UStaticArtifactData* Data, const TArray<UPlayerUnitModel*>& PartyMembers);
	 ...
	 */
	bool AddArtifact(const FPrimaryAssetId& ArtifactId, const TArray<UPlayerUnitModel*>& PartyMembers);
	 ...
	 */
	bool RemoveArtifact(UStaticArtifactData* Data, const TArray<UPlayerUnitModel*>& PartyMembers);
