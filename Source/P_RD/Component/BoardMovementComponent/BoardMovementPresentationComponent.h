...
/**
 * @brief  보드 액터 이동 연출 컴포넌트
 *
 * @details
 * 이동 컴포넌트 모델이 발행하는 델리게이트(배치/경로/스텝/회전/텔레포트)를 받아 소유 액터를 움직임
 * 경로 추종(폴리라인 코너링), 가감속, 배리어 해제 등 화면 표현만 담당
 * 이동 판정은 모델이 이미 끝냈으므로 충돌 검사 없이 트랜스폼만 보간
 */
UCLASS(ClassGroup = (BoardMovement), meta = (BlueprintSpawnableComponent))
class P_RD_API UBoardMovementPresentationComponent : public UActorComponent, public IComponentView
{
	GENERATED_BODY()

public:
	UBoardMovementPresentationComponent();

	/* IComponentView 상속 */
public:
	// @brief 이동 델리게이트 구독
	void BindOwnerModel(UObjectModel* Model) override;
	// @brief 이동 델리게이트 구독 해제 + 연출 상태 정리
	void UnbindOwnerModel(UObjectModel* Model) override;

	/* UActorComponent 상속 */
public:
	// @brief 이동 연출 틱 (평소엔 꺼두고 연출 중에만 켬)
...
public:
	...
	FVector GetMoveVelocity() const;
	// @brief 바닥 기준 높이 오프셋 설정 (유닛=캡슐 반높이, 장애물=0)
	void SetGroundZOffset(float Offset);
...
private:
	...
	// @brief 직선(폴리라인 없음) 이동 연출
	void TickStraight(float DeltaSeconds);
...
protected:
	...
	// @brief 바닥 기준 높이 오프셋 (cm). 타일 트랜스폼은 바닥 기준이라 액터 중심을 이만큼 올려서 세움
	UPROPERTY(Category = Move, EditAnywhere, BlueprintReadWrite, meta = (DisplayName = "GroundZOffset"))
	float mGroundZOffset = 0.0f;

private:
	// @brief 구독 중인 보드 액터 모델 (구독 해제용)
	TWeakObjectPtr<UBoardActorModel> mBoardActorModel;
