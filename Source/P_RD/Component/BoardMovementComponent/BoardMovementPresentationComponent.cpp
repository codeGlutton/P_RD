...
UBoardMovementPresentationComponent::UBoardMovementPresentationComponent()
{
	// @brief 이동 연출용 틱
...
void UBoardMovementPresentationComponent::BindOwnerModel(UObjectModel* Model)
{
	mBoardActorModel = Cast<UBoardActorModel>(Model);
	if (mBoardActorModel.IsValid() == false)
	{
		return;
	}
	...
	// 텔레포트 연출 요청 구독
	mBoardActorModel->OnTeleport.AddUObject(this, &UBoardMovementPresentationComponent::OnTeleport);
}
...
void UBoardMovementPresentationComponent::UnbindOwnerModel(UObjectModel* Model)
{
	if (mBoardActorModel.IsValid())
	{
		...
		mBoardActorModel->OnTeleport.RemoveAll(this);
	}
...
void UBoardMovementPresentationComponent::OnRotate(const FRotator& TargetWorldRotation, TSharedPtr<FPresentationBarrier> Barrier)
{
	AActor* Owner = GetOwner();
	if (Owner == nullptr)
	{
	...
	// 목표를 "현재 위치 + 새 회전"으로 잡아서 틱에서 처리하도록 설정
	mMoveTargetTransform = FTransform(TargetWorldRotation, Owner->GetActorLocation());
	// 인자로 들어온 배리어가 nullptr이면
	// -> 틱이 배리어를 요구하므로 빈 배리어를 만들어서 처리
...
void UBoardMovementPresentationComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (mPolyLinePoints.Num() >= 2)
	{
		TickPolyLine(DeltaTime);
		return;
	}

	// 폴리라인이 없으면 직선이동모드로 처리
	TickStraight(DeltaTime);
}

void UBoardMovementPresentationComponent::TickStraight(float DeltaSeconds)
{
	AActor* Owner = GetOwner();
	if (Owner == nullptr)
	{
		...
		NewRotation.Equals(mMoveTargetTransform.Rotator()))
	{
		...
		SetComponentTickEnabled(false);
		// 베리어 해제
		// -> 이동 컴포넌트 모델의 OnStepPresentationFinished() 호출
		// -> StartStep(NextTile) 해서 타일간 이동 반복
		mMoveBarrier.Reset();
	}
}

void UBoardMovementPresentationComponent::TickPolyLine(float DeltaSeconds)
{
	AActor* Owner = GetOwner();
	if (Owner == nullptr)
	{
	...
	// 폴리라인은 타일 바닥 기준이므로 바닥 오프셋만큼 보정
	SampleLocation.Z += mGroundZOffset;
	...
	const FRotator NewRotation = FMath::RInterpConstantTo(Owner->GetActorRotation(), TargetRotation, DeltaSeconds, mRotationSpeed);
	Owner->SetActorLocationAndRotation(SampleLocation, NewRotation);
...
bool UBoardMovementPresentationComponent::GetPolyLinePoint(float Distance, FVector& OutLocation, FVector& OutTangent) const
{
...
void UBoardMovementPresentationComponent::ResetPolyLineState()
{
...
void UBoardMovementPresentationComponent::BakePolyLinePoints(
...
void UBoardMovementPresentationComponent::SetGroundZOffset(float Offset)
{
	mGroundZOffset = Offset;
}
