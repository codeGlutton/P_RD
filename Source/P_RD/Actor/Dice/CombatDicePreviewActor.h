...
struct FRDCombatDiceFaceTextureSet
{
	...
	/** @brief FaceIndex 0-base 순서의 기본 면 커버 텍스처 캐시. */
	UPROPERTY(Transient)
...
public:
	/** @brief 컴포넌트/머티리얼/에셋 참조만 준비하고 RegisterComponent가 필요한 작업은 런타임으로 미룬다. */
	ACombatDicePreviewActor();

	/** @brief 주사위 종류(면 수)에 맞는 다면체 메시 + 면 숫자 TextRender를 다시 만든다. */
	// FaceCount가 미지원이면 메시/다면체 데이터 모두 d6 fallback을 사용한다.
...
public:
	...
	// FaceValue는 UI/게임플레이가 보는 1-base 눈 값이고, 내부 ValueFaceIndex는 FaceValue-1이다.
	...
	// NewFaceValues는 물리 면 0-base 순서다. UDiceModel::mFaceValues와 같은 배열 계약을 따른다.
	void SetFaceValues(const TArray<int32>& NewFaceValues);

	/** @brief 각 면에 덮을 텍스처를 갱신한다(nullptr이면 기본 면 텍스처). */
	// NewFaceTextures도 물리 면 0-base 순서다. 부족한 값은 기본 텍스처로 fallback한다.
...
private:
	/** @brief 생성자에서 안전하게 만들 수 있는 Scene/StaticMesh/Light 컴포넌트 트리를 구성한다. */
	void InitializeSceneComponents();
	/** @brief 캡처/뷰포트에서 어두운 면이 죽지 않도록 보조 조명만 구성한다. */
	void InitializeLighting();
	/** @brief TextRender 숫자 표시용 엔진 폰트를 로드한다. */
	void LoadDiceNumberFont();
	/** @brief 지원 면 수별 블랭크 메시 에셋을 로드해 mDiceMeshAssets에 캐시한다. */
	void LoadDiceMeshes();
	/** @brief 몸체/림/면 커버용 동적 머티리얼 템플릿을 준비한다. */
	void InitializeMaterials();
	/** @brief 면 수별 기본 커버 텍스처를 로드해 런타임 override 누락 시 fallback으로 쓴다. */
	...
	// 외부 FaceIndex는 눈 값과 맞춘 1-base, mFaceTexts 접근은 0-base로 변환한다.
...
private:
	...
	/** @brief 각 숫자 면 위의 커버 절차 메시. mFaceMaterials와 같은 FaceIndex 순서다. */
	UPROPERTY(Transient)
	...
	// [합의필요] UI 패널 실제 크기가 확정되면 캡처 카메라 거리/FOV와 함께 조정할 수 있다.
