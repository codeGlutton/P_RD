...
/** @brief 생성자에서 안전한 기본 컴포넌트 트리를 만든다. 동적 면 커버/TextRender는 여기서 만들지 않는다. */
...
/** @brief 지원 면 수별 블랭크 메시를 ConstructorHelpers로 로드해 면 수 map에 캐시한다. */
...
/** @brief 캡처/Viewport에서 주사위 면이 검게 죽지 않도록 고정 보조 조명을 배치한다. */
...
/** @brief TextRender 숫자에 사용할 DistanceField 폰트를 로드한다. */
