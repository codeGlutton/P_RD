...
public:
	...
	/** @brief 전투 HUD가 뷰모델에서 읽은 Lv/HP/Gold를 탑바 요약에 직접 반영한다. */
	// 게임플레이 ASC 속성 동기화(RunPersistData)가 깨져 쓰레기값을 주므로, 전투 중에는 전투 뷰모델 값을
	// 정확한 소스로 삼아 탑바가 같은 Lv/HP/Gold를 보이게 한다. 속성 초기화가 고쳐지면 이 경로는 정리 가능.
