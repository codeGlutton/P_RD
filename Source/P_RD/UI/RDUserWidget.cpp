...
namespace
{
	// 공용 버튼 클릭 사운드(SVN 관리 uasset). 문자열 로드 -> DefaultGame.ini의 DirectoriesToAlwaysCook(.../Audio)로 쿡 보장.
	const TCHAR* const CommonButtonPressSoundPath = TEXT("/Game/SVN/OutSideAsset/AICreation/Audio/UISFX/SFX_UI_Click_Scratch003.SFX_UI_Click_Scratch003");
	...
	// 한 번 로드하면 버튼 스타일이 참조를 잡아 살아 있으므로 약참조 캐시로 충분하다.
	USoundBase* GetCommonButtonPressSound()
	{
		static TWeakObjectPtr<USoundBase> CachedSound;
		if (CachedSound.IsValid() == false)
		{
			CachedSound = LoadObject<USoundBase>(nullptr, CommonButtonPressSoundPath);
		}
...
/**
 * @brief 초기화 시 이 위젯 트리의 모든 버튼에 공용 클릭 사운드 + 누름 스케일 효과를 1회 배선한다.
 */
...
void URDUserWidget::SetupCommonButtonFeedback()
{
	...
	USoundBase* PressSound = GetCommonButtonPressSound();
