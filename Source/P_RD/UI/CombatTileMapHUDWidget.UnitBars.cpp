...
void UCombatTileMapHUDWidget::RebuildUnitHpBars()
{
	...
	const int32 DesiredCount = mCombatViewModel != nullptr ? mCombatViewModel->GetUnitViews().Num() : 0;
...
void UCombatTileMapHUDWidget::UpdateUnitHpBars()
{
	...
	const TArray<FUnitView>& Units = mCombatViewModel->GetUnitViews();
	for (int32 BarIndex = 0; BarIndex < mUnitHpBars.Num(); ++BarIndex)
	{
		...
		const FUnitView& Unit = Units[BarIndex];
