...
void UShopUIWidgetBase::HandleBuyClicked()
{
	...
	{
		const FShopOwnedUnitUI& Unit = Shop.mSkillTargetUnits[mSelectedUnitViewIndex];
		if (!Unit.mIsOwned || Unit.mUnitIndex == INDEX_NONE)
		{
			return;
		}
...
{
	if (mUIModel == nullptr
		|| !mUIModel->GetShop().mSkillTargetUnits.IsValidIndex(mSelectedUnitViewIndex)
		|| !mUIModel->GetShop().mSkillTargetUnits[mSelectedUnitViewIndex]
	...
	const FShopItemUI& Item) const
{
	...
	const FShopOwnedUnitUI& SelectedUnit =
		Shop.mSkillTargetUnits[mSelectedUnitViewIndex];
	if (!SelectedUnit.mIsOwned)
	{
...
void UShopUIWidgetBase::RefreshSelectedItemView(const FShopUI& Shop)
{
	...
	if (Item->mKind == EShopItemKind::Skill)
	{
		bHasSkillTarget = Shop.mSkillTargetUnits.IsValidIndex(mSelectedUnitViewIndex)
			&& Shop.mSkillTargetUnits[mSelectedUnitViewIndex].mIsOwned
			&& Shop.mSkillTargetUnits[mSelectedUnitViewIndex]
