 ...
 * 상세 패널을 무엇으로 어떻게 띄울지는 게임플레이가 결정한다 — UIModel은 순수 신호만 보낸다.
 */
void UInventoryUIModel::RequestItemLongPress(EInventoryItemKind Kind, int32 ItemIndex)
{
	OnItemLongPress.Broadcast(Kind, ItemIndex);
}

/**
 * @details 게임플레이가 모은 런 상태 스냅샷을 저장하고 변경 알림을 쏜다.
