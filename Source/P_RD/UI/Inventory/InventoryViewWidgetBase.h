// @file InventoryViewWidgetBase.h
...
#include "UI/Inventory/InventoryViewTypes.h"
#include "InventoryViewWidgetBase.generated.h"

class UInventoryViewModel;

/** @brief 인벤토리 화면 WBP 베이스. 이 클래스를 상속한 WBP가 실제 비주얼을 그린다. */
// 이 베이스를 상속한 WBP는:
// - BindViewModel()로 UInventoryViewModel에 연결하고,
// - OnInventoryRefreshed(BlueprintImplementableEvent)에서 GetViewModel()->GetInventory()를 읽어 목록을 그리고,
// - 항목 롱프레스는 RequestItemLongPress()로 의도만 보낸다.
// 위젯은 런 상태를 계산/변경하지 않는다. 진실은 게임플레이에 있다.
UCLASS(Abstract)
class P_RD_API UInventoryViewWidgetBase : public URDUserWidget
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Inventory|View")
	void BindViewModel(UInventoryViewModel* InViewModel);

	/** @brief WBP가 목록을 그릴 때 현재 ViewModel을 읽기 위한 접근자. */
	UFUNCTION(BlueprintPure, Category = "Inventory|View")
	UInventoryViewModel* GetViewModel() const { return mViewModel; }
...
protected:
	...
	UFUNCTION(BlueprintImplementableEvent, Category = "Inventory|View")
...
protected:
	...
	TObjectPtr<UInventoryViewModel> mViewModel;
};
