...
namespace MercenaryHireDetail
{
	...
	const TCHAR* HeroPaths[CardCount] = {
		...
		TEXT("/Game/SVN/OutSideAsset/AICreation/UI/P_RD/Characters/Mercenaries/T_MB_HireHero_Mage.T_MB_HireHero_Mage"),
		TEXT("/Game/SVN/OutSideAsset/AICreation/UI/P_RD/Characters/Mercenaries/T_MB_HireHero_Ranger.T_MB_HireHero_Ranger"),
		TEXT("/Game/SVN/OutSideAsset/AICreation/UI/P_RD/Characters/Mercenaries/T_MB_HireHero_Rogue.T_MB_HireHero_Rogue"),
		TEXT("/Game/SVN/OutSideAsset/AICreation/UI/P_RD/Characters/Mercenaries/T_MB_HireHero_Barbarian.T_MB_HireHero_Barbarian"),
		TEXT("/Game/SVN/OutSideAsset/AICreation/UI/P_RD/Characters/Mercenaries/T_MB_HireHero_Druid.T_MB_HireHero_Druid")
	};
...
void UMercenaryHireWidget::ApplyMarchboundPortraits()
{
	...
	for (FFrontendCharacterOption& Option : mCrew)
	{
		...
		// Marchbound 화면 전용 아트만 교체한다. 후보 수·스탯·선택 가능 여부는
		// FrontendGameMode가 내려준 값을 그대로 신뢰한다.
...
void UMercenaryHireWidget::ClickCard(const int32 CardIndex)
{
	...
	// 명시적으로 수행해, 클래스를 둘러보다가 파티가 바뀌는 일을 막는다.
...
void UMercenaryHireWidget::ClickPartySlot(const int32 SlotIndex)
{
	...
	{
		// 빈 파티 슬롯은 자리 표시만 한다. 추가는 중앙의 명시적 버튼만 맡는다.
		return;
	}
 ...
 * 상세를 둘러보는 입력과 파티 편성 입력을 분리해 의도치 않은 추가를 막는다.
...
void UMercenaryHireWidget::RefreshBottomBar()
{
	...
	for (int32 SlotIndex = 0; SlotIndex < mSlots.Num(); ++SlotIndex)
	{
		...
		if (!bFilled)
		{
			MercenaryHireDetail::SetTextIfPresent(Widgets.mName, FText::GetEmpty());
			continue;
		}
