...
void UDicePanelWidget::EnsureDiceCarouselControlWidgets()
{
	...
	if (mDiceCarouselFaceButtons.Num() != DicePanelSlotCount)
	{
		mDiceCarouselFaceButtons.SetNum(DicePanelSlotCount);
	}
	if (mDiceCarouselFaceButtonTexts.Num() != DicePanelSlotCount)
	{
		mDiceCarouselFaceButtonTexts.SetNum(DicePanelSlotCount);
	}

	for (int32 FaceIndex = 0; FaceIndex < DicePanelSlotCount; ++FaceIndex)
	{
		...
		UButton* FaceButton = WidgetTree->ConstructWidget<UButton>(
			UButton::StaticClass(),
...
void UDicePanelWidget::RefreshDiceCarouselControlWidgets()
{
	...
	for (UButton* FaceButton : mDiceCarouselFaceButtons)
	{
		if (FaceButton != nullptr)
		{
			FaceButton->SetVisibility(ControlVisibility);
		}
	}
	if (mDiceCarouselPreviousButton != nullptr)
	{
	...
	if (mDiceCarouselFaceText != nullptr)
	{
		mDiceCarouselFaceText->SetText(FText::Format(
			NSLOCTEXT("DicePanelWidget", "DiceCarouselFaceFormat", "RESULT {0}"),
...
void UDicePanelWidget::RefreshDiceCarouselFaceButtonStyles()
{
	...
	{
		UButton* FaceButton = mDiceCarouselFaceButtons[FaceIndex];
		if (FaceButton == nullptr)
		{
			continue;
		}
