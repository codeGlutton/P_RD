...
void URunOptionsRailWidget::RefreshMercenaryPanel()
{
	...
	// (codeGlutton PR #426 규칙, GetInventoryView와 같은 idiom).
	PartyRoster = FPartyRosterView();
...
void URunOptionsRailWidget::RefreshInventoryPage()
{
	...
	ARoomGameModeBase* GameMode = GetWorld() != nullptr
		? GetWorld()->GetAuthGameMode<ARoomGameModeBase>() : nullptr;
	// 가방도 밀지 않고 물어본다 -- GameMode의 GetInventoryView 하나로 받는다.
	FInventoryView Inventory;
	if (GameMode == nullptr || GameMode->GetInventoryView(OUT Inventory) == false)
	{
		return;
	}
	...
	ShownArtifactViews.Reset();
	ShownArtifactIndices.Reset();
	for (const FInventoryArtifactView& Artifact : Inventory.mArtifacts)
	{
	...
	for (int32 Index = 0; Index < 7; ++Index)
	{
		const FInventoryArtifactView* Artifact = ShownArtifactViews.IsValidIndex(Index)
...
void URunOptionsRailWidget::ShowArtifactDetail(const int32 SlotIndex)
{
	 ...
	 */
	const FInventoryArtifactView& Artifact = ShownArtifactViews[SlotIndex];
