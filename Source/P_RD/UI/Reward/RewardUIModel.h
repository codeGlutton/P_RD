...
// 돈/경험치와 룸이 지급하는 항목 보상 표시값을 UI로 전달한다.
...
public:
	...
	/** @brief 위젯이 보상 항목 하나를 골랐음(ChoiceIndex). 게임플레이가 실제 보상을 지급한다. */
	UPROPERTY(BlueprintAssignable, Category = "Reward|Input")
	FOnRewardChosen OnRewardChosen;

	/** @brief 위젯이 보상 행 하나를 눌렀음. 게임플레이가 행 종류에 맞는 실제 보상을 지급한다. */
	UPROPERTY(BlueprintAssignable, Category = "Reward|Input")
...
public:
	...
	/** @brief 보상 항목에서 ChoiceIndex를 골랐다는 의도를 게임플레이 구독자에게 전달한다. */
