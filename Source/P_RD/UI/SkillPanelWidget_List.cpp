...
namespace
{
	...
	UButton* FindOrCreateSkillSelectButton(UWidgetTree* WidgetTree, UCanvasPanel* RootCanvas, FName Name)
	{
		...
		if (UButton* ExistingButton = Cast<UButton>(WidgetTree->FindWidget(Name)))
		{
			return ExistingButton;
		}

		UButton* NewButton = WidgetTree->ConstructWidget<UButton>(UButton::StaticClass(), Name);
		if (NewButton != nullptr && RootCanvas != nullptr)
		{
			RootCanvas->AddChildToCanvas(NewButton);
		}
		return NewButton;
	}
}
...
void USkillPanelWidget::EnsureSkillSelectButtons()
{
	...
	for (int32 Index = 0; Index < RDSkillPanel::ItemCount; ++Index)
	{
		...
		UButton* SelectButton = FindOrCreateSkillSelectButton(
			WidgetTree,
			mRootCanvas,
			FName(*FString::Printf(TEXT("SkillSelectButton_%d"), Index))
		);
