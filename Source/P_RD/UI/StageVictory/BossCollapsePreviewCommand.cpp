...
namespace
{
    TWeakObjectPtr<UBossCollapseWidget> Preview;
    void PlayPreview(UWorld* World, int32 Stage, bool All, bool Verify)
    {
        auto* Player=World?World->GetFirstPlayerController():nullptr;
        if(!Player)return;
        if(Preview.IsValid())Preview->Cancel();
        TWeakObjectPtr<UWorld> WeakWorld=World;
        Preview=UBossCollapseWidget::Show(Player,Stage,FSimpleDelegate::CreateWeakLambda(Player,[WeakWorld,Stage,All,Verify]()
        {
            Preview.Reset();
            ...
            UE_LOG(LogTemp,Display,TEXT("RD_BOSS_COLLAPSE preview complete all=%d"),All);
            if(Verify)FPlatformMisc::RequestExit(false);
        }));
        if(!Preview.IsValid())
        {
            UE_LOG(LogTemp,Error,TEXT("RD_BOSS_COLLAPSE preview failed stage=%d"),Stage);
            if(Verify)FPlatformMisc::RequestExit(false);
            return;
        }
        if(Verify)
        {
            const float Duration=Preview->GetDuration();
            for(int32 Beat=0;Beat<2;++Beat)
            {
                ...
                {
                    FScreenshotRequest::RequestScreenshot(FPaths::Combine(FPaths::ProjectSavedDir(),TEXT("Screenshots"),FString::Printf(TEXT("BossCollapse_Stage%d_Beat%d.png"),Stage,Beat)),true,false);
                }),Beat==0?.85f:Duration-.35f,false);
            }
        }
    }
    FAutoConsoleCommandWithWorldAndArgs Command(TEXT("RD.BossCollapse.Preview"),
        TEXT("Boss victory comic only; no rewards or saves. Stage 1-3, or 0 for all. Optional verify captures and exits."),
