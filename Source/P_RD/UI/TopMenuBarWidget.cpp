...
/**
 * @brief 탑바가 다른 팝업 위에서 보이도록 표시 순서를 설정한다.
 *
 * @details
 * 월드맵/설정 패널도 팝업 표시 순서를 사용하므로, 탑바는 PopUp보다 한 단계 위에 둔다.
 ...
 *
 * @details
 * OpenUI 완료 후 프레젠테이션 배리어를 해제해 전투 종료 연출과 다음 방 선택 UI의 경계가 끊기지 않게 한다.
 *
...
/**
 * @brief 설정 패널의 뒤로가기 요청을 현재 지도 잠금 상태에 맞게 처리한다.
 */
