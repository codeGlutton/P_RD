...
void UCombatLayoutHUDWidget::CacheAuthoredWidgets()
{
	...
	{
		FQuickSkillWidgets& Widgets = mQuickSkillSlots[Index];
		const FString Suffix = FString::Printf(TEXT("_%d"), Index);
		Widgets.Root = Find<UWidget>(WidgetTree, TEXT("QuickSkillSlot") + Suffix);
		Widgets.Icon = Find<UImage>(WidgetTree, TEXT("QuickSkillIcon") + Suffix);
		Widgets.Cooldown = Find<UTextBlock>(
			WidgetTree, TEXT("QuickSkillCooldown") + Suffix);
		Widgets.Button = Find<UButton>(WidgetTree, TEXT("QuickSkillButton") + Suffix);
	}
...
void UCombatLayoutHUDWidget::HandleAnyPressed()
{
	...
	for (int32 SlotIndex = 0; SlotIndex < mCommandSlots.Num(); ++SlotIndex)
	{
		...
		if (mQuickSkillSlots.IsValidIndex(SlotIndex)
			&& mQuickSkillSlots[SlotIndex].Button != nullptr
			&& mQuickSkillSlots[SlotIndex].Button->IsPressed())
		{
			CommandButton = mQuickSkillSlots[SlotIndex].Button;
		}
...
void UCombatLayoutHUDWidget::NativeOnUIRefreshed(const ECombatUIDomain Domain)
{
	...
	{
		// 차례가 바뀌면 지난 카드 상태를 접는다. 하단 퀵바가 상시 선택 경로를
		// 제공하므로 큰 카드 고리는 사용자가 스킬 버튼을 눌렀을 때만 연다.
...
void UCombatLayoutHUDWidget::RefreshCommands()
{
	...
	for (int32 SlotIndex = 0; SlotIndex < mCommandSlots.Num(); ++SlotIndex)
	{
		...
		// 기존 우하단 배지의 남은 턴 표기는 그대로 유지한다. 하단 퀵바와
		// 같은 중앙 숫자는 먼 거리에서도 쿨타임을 읽게 하는 추가 피드백이지,
		// 기존 카드 정보를 대체하지 않는다.
	...
	RefreshQuickSkillBar();
}

void UCombatLayoutHUDWidget::RefreshQuickSkillBar()
{
	if (mUIModel == nullptr)
	{
		return;
	...
	for (int32 Index = 0; Index < mQuickSkillSlots.Num(); ++Index)
	{
		const FQuickSkillWidgets& Widgets = mQuickSkillSlots[Index];
		if (Index == 0)
		{
			SetShown(Widgets.Root, true);
			if (Widgets.Icon != nullptr)
			{
				...
				const bool bCanMove = mUIModel->IsSkillRailOwnTurn()
					&& TurnUnit != nullptr
					&& FMath::RoundToInt(TurnUnit->mMovementPoint) > 0;
				if (mCommandMoveIconTexture != nullptr)
				{
					Widgets.Icon->SetBrushFromTexture(mCommandMoveIconTexture, false);
					FSlateBrush Brush = Widgets.Icon->GetBrush();
					Brush.DrawAs = ESlateBrushDrawType::Image;
					Widgets.Icon->SetBrush(Brush);
				}
				Widgets.Icon->SetColorAndOpacity(bCanMove
					? FLinearColor::White : FLinearColor(.46f, .46f, .46f, 1.f));
				SetShown(Widgets.Icon, mCommandMoveIconTexture != nullptr);
			}
			SetShown(Widgets.Cooldown, false);
			if (Widgets.Button != nullptr)
			{
		...
		const bool bHasSkill = Skills.IsValidIndex(SkillIndex)
			&& Skills[SkillIndex].mIcon != nullptr;
		SetShown(Widgets.Root, bHasSkill);
		if (!bHasSkill)
		{
			continue;
		}
		...
		if (Widgets.Icon != nullptr)
		{
			...
			Brush.DrawAs = ESlateBrushDrawType::Image;
			Widgets.Icon->SetBrush(Brush);
			const float Brightness = Skill.mRemainingCooldown > 0
				? .22f : (Skill.mIsUsable ? 1.f : .46f);
			Widgets.Icon->SetColorAndOpacity(FLinearColor(
				Brightness, Brightness, Brightness, 1.f));
			SetShown(Widgets.Icon, true);
		...
		if (Widgets.Cooldown != nullptr)
		{
			const bool bOnCooldown = Skill.mRemainingCooldown > 0;
			SetShown(Widgets.Cooldown, bOnCooldown);
			Widgets.Cooldown->SetText(FText::AsNumber(Skill.mRemainingCooldown));
		}
		if (Widgets.Button != nullptr)
		{
			// 사용 불가 상태도 긴 눌러 상세를 볼 수 있어야 하므로 입력 자체는
			// 유지하고 RequestCommand가 실제 선택만 거른다.
			Widgets.Button->SetIsEnabled(true);
		}
	}
}
...
void UCombatLayoutHUDWidget::CompleteTurnPresentationBegin()
{
	...
	// 큰 카드 고리는 자동으로 열지 않는다. 퀵바는 RefreshCommandVisibility가
	// 켜고, 카드는 오른쪽 스킬 버튼을 누를 때만 열린다.
...
void UCombatLayoutHUDWidget::RefreshActionButtons()
{
	...
	// 숨기고, 왼쪽의 작고 붉은 전용 취소 단추만 켜 근육 기억 실수를 막는다.
	...
	const bool bSkillButtonShown = bBrowsing;
 ...
 *
 * @details
 * 남은 칸의 **왼쪽부터** 빛낸다 -- 실제 소모 표시도 왼쪽부터 비우므로,
 * 없어질 그 칸이 빛나야 "이만큼 나간다" 로 읽힌다.
 *
...
void UCombatLayoutHUDWidget::RefreshPendingAPGlow(const float DeltaTime)
{
	...
	const int32 ShownLeft = FMath::Min(Left, Room);
	// 10칸을 넘는 AP는 숫자만 맡지만 예정 비용 피드백까지 숨기면 안 된다.
	// 예: 12/12에서 비용 1~2인 이동은 예전 계산상 표시 칸이 0이 되어 아무
	// 보석도 번쩍이지 않았다. 실제 칸 감소 여부와 별개로, 현재 보이는 묶음의
...
{
	const bool bQuickBarVisible = IsPlayerTurn() == true
		&& mIsTurnActive == true
		&& mIsActionPlaying == false
		&& mCommandsShown == false
		&& IsAiming() == false
		...
		&& IsDetailOverlayShown() == false;
	SetShown(mQuickSkillBar, bQuickBarVisible);
