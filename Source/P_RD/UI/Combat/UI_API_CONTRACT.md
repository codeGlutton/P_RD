...
> 작성: 박용수(UI/Dice) · 기준 코드: `UCombatViewModel` / `CombatViewTypes.h`
> 회의(2026-06-15) 합의 = **데이터/비주얼 분리 + MVVM**. UI는 게임플레이 객체(`UUnitData`/`UDiceData`/`ATileMap`…)를 **직접 알지 않고**, 이 문서의 뷰 타입과 `UCombatViewModel` 계약만으로 동작한다.

전투 UI와 게임플레이는 **`UCombatViewModel` 한 곳**에서만 만난다. 소유 = `USRPGCombatSubsystem`(전투 수명). HUD는 `BindCombatViewModel(VM)` 하나로 붙는다.
...
게임플레이/어댑터가 표시값을 `Set*()`로 push → VM이 `OnViewChanged(도메인)` 발신 → 위젯이 해당 도메인만 다시 그림. **UI는 이걸 못 만들어내므로 게임플레이가 반드시 공급해야 한다.**

| 도메인 | Set 함수 | 뷰 타입 | 게임플레이가 채워야 할 핵심 필드 |
|---|---|---|---|
| Unit | `SetUnitViews(TArray<FUnitView>)` | `FUnitView` | `mUnitId, mIsPlayer, mHP/mMaxHP, mMovementPoint/mMaxMovementPoint, mTile, mWorldLocation`(머리위 HP바 투영용), `mStatusTags`(버프/디버프, enum 아닌 태그) |
| Unit(상세) | `SetUnitDetail(FUnitDetailView)` | `FUnitDetailView` | 롱프레스 시 `mName, mLevel, mPortrait, mHP/mMaxHP, mPassiveDescriptions` |
| Dice | `SetDiceViews(TArray<FDiceSlotView>)` | `FDiceSlotView` | `mDiceId, mResultValue`(0=미굴림), `mIsRolled, mIsSelected, mIsUsed`(이번턴 잠금), `mRarityColor/mRarityText`(어댑터가 미리 색/문구로 변환), `mPreviewTexture`(주사위 3D 캡처 결과) |
| Dice(선택) | `SetSelectedDice(TArray<int32>, int32 Sum)` | — | 스킬 빌드에 올린 주사위 인덱스들 + 합계 |
| Skill | `SetSkillViews(TArray<FSkillView>)` | `FSkillView` | `mSkillIndex, mName, mIcon, mDiceCost, mIsUsable` |
| Skill(상세) | `SetSkillDetail(FSkillDetailView)` | `FSkillDetailView` | 롱프레스 시 `mDescription` 등 |
| Tile | `SetTileStates(TArray<FTileViewState>)` | `FTileViewState` | `mTile, mHighlightFlag`(=`ETileHighlightFlag` 비트), `mOccupantUnitId` |
| Turn | `SetTurnView(FTurnView)` | `FTurnView` | `mCurrentUnitId, mRound, mPhase`(=`ECombatBuildPhaseView`, develop `ESRPGSkillBuildPhase`의 거울), `mTurnOrderUnitIds` |
| Equipment | `SetEquipmentViews(TArray<FEquipmentView>)` | `FEquipmentView` | `mSlotIndex, mItemId, mName, mIcon, mIsEquipped, mRarityColor` |
| Meta | `SetPlayerMeta(FPlayerMetaView)` | `FPlayerMetaView` | `mGold, mLevel, mExp/mMaxExp` (상단 상태바·보상) |
...
| 주사위 굴림 | `RequestRollDice()` | `OnCombatCommand(RollDice, 0)` | 없음 |
| 이동 모드 | `RequestMove()` | `OnCombatCommand(Move, 0)` | 없음 |
| 턴 종료 | `RequestEndTurn()` | `OnCombatCommand(EndTurn, 0)` | 없음 |
| 취소(딴 데 탭) | `RequestCancel()` | `OnCombatCommand(Cancel, 0)` | 없음 |
...
- `OnViewChanged(ECombatViewDomain)` — 바뀐 도메인만 부분 갱신.
...
현재는 비GAS **임시 어댑터**(`UCombatViewAdapter`/`UDiceAdapter`)가 A의 `Set*`를 채우고 B의 입력을 처리한다(플레이스홀더·가상 적 포함). 실제 게임플레이 연결 시 어댑터 자리를 다음으로 대체:
...
- **스킬 빌드 페이즈**(A.Turn.mPhase) ← develop `ESRPGSkillBuildPhase` 미러. `ACombatGameMode::SelectSkill`/`OnChangeSkillBuildPhase`와 연동.
...
- 적 정보 표시 = 작은 팝업 ❌ → **크게 뜨는 정보 패널**(회의 합의) 로 `FUnitDetailView` 소비.
- 주사위 최대 개수 제한 여부(보유 다이스 레이아웃에 영향).
- `mPhase`/`ECombatBuildPhaseView` ↔ develop `ESRPGSkillBuildPhase` 값 1:1 확정.
