...
/**
 * @brief 탑바 DICE 버튼으로 여는 보유 주사위 확인 패널
 *
 * @details
...
private:
	...
	void BindDiceCarouselFaceButton(UButton* FaceButton, int32 FaceValue);

	/** @brief 주사위 면 버튼의 해당 면 값 핸들러 연결을 해제한다. */
	void UnbindDiceCarouselFaceButton(UButton* FaceButton, int32 FaceValue);
...
private:
	...
	UPROPERTY(Transient)
	TArray<TObjectPtr<UButton>> mDiceCarouselFaceButtons;
