...
void UTopMenuBarWidget::RefreshRoomInfo() const
{
	...
	if (SummaryTextBlock != nullptr)
	{
		// 기본 요약(Lv/난이도). 전투 중에는 전투 HUD가 SetCombatPlayerSummary로 Lv/HP/Gold를 덮어쓴다.
		// (RunPersistData의 HP/Money는 ASC 속성 동기화가 깨져 쓰레기값이라 여기서 직접 읽지 않는다.)
...
{
	// 전투 HUD가 뷰모델에서 읽은 Lv/HP/Gold를 탑바 요약에 직접 반영한다.
	// (게임플레이 ASC 속성 동기화가 깨져 RunPersistData를 못 믿으므로, 전투 중엔 이 값이 진짜 소스.)
	if (SummaryTextBlock == nullptr)
	{
		return;
	}
	SummaryTextBlock->SetText(FText::Format(
		NSLOCTEXT("TopMenuBarWidget", "CombatPlayerSummaryFormat", "Lv {0}    HP {1}/{2}    GOLD {3}"),
