 ...
 *
 * @details
 * 이 파일의 타입들은 feature/create-srpg-framework-base 브랜치에서 가져온 원본 게임 데이터가 아니다.
 * PM 브랜치의 원본 데이터는 FStage, FRoom, URunPersistData, RoomTransitionSubsystem 쪽에 있고,
 * 이 파일은 그 원본 값을 WBP가 바로 표시할 수 있는 형태로 옮겨 담는 View 전용 DTO를 정의한다.
 *
 * 즉 책임 경계는 다음과 같다.
 * - PM API: Stage/Room 생성, 현재 룸 위치, 룸 전환, 저장/런 상태 같은 실제 게임 데이터와 규칙을 담당한다.
 * - FrontendViewTypes: PM API 결과를 UI가 노드/선/버튼 상태로 그릴 수 있게 변환한 읽기 전용 표시 데이터를 담는다.
 *
 ...
 *
 * 예를 들어 FRoom::mType과 FRoom::mNextRoomColumns는 PM 데이터에서 오지만,
 ...
 * 게임 상태 변경 책임이 화면 코드로 넘어간다. 그래서 FrontendGameMode가 PM 브랜치의 실제 상태를 읽고,
