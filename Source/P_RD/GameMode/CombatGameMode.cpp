...
void ACombatGameMode::OnRegisterUnit(UUnitModel* Unit)
{
	...
	if (AttributeComponentModel != nullptr)
	{
		const TArray<FTacticalAttribute> AttributesToRefresh = {
			...
			UUnitAttributeSet::GetDamagePointAttribute()
		};
...
void ACombatGameMode::PushUnitUI()
{
	...
	for (const TObjectPtr<UUnitModel>& Unit : CombatModel->GetUnits())
	{
		...
		if (UAttributeSetComponentModel* AttributeComponentModel = Unit->GetAttributeComponentModel())
		{
			...
			UnitUI.mDamagePoint = AttributeComponentModel->GetAttributeCurrentValue(UUnitAttributeSet::GetDamagePointAttribute());
...
void ACombatGameMode::PushSkillUI()
{
	...
	const int32 SkillCount = SkillComponentModel->GetSkillDataNum();
	...
	for (int32 SkillIndex = 0; SkillIndex < SkillCount; ++SkillIndex)
	{
		...
		TSoftObjectPtr<UStaticSkillData> StaticSkillDataPtr;
		if (SkillComponentModel->TryGetSkillData(SkillIndex, OUT StaticSkillDataPtr) == false)
		{
		...
		UStaticSkillData* StaticSkillData = StaticSkillDataPtr.LoadSynchronous();
		if (StaticSkillData == nullptr)
		{
			UE_LOG(
				LogCombatGameMode,
				Warning,
				TEXT("CombatGameMode: PushSkillUI failed to load StaticSkillData at slot %d. Path=%s"),
				SkillIndex,
				*StaticSkillDataPtr.ToSoftObjectPath().ToString());
			SkillUIs.Add(SkillUI);
			continue;
		}
		...
		SkillUI.mDiceCost = StaticSkillData->mDiceCount;
		SkillUI.mIsUsable = true;
		SkillUI.mTargeting.mSelectShape = ToCombatSkillSelectShape(StaticSkillData->mAimPattern);
		SkillUI.mTargeting.mSelectRange = StaticCast<float>(StaticSkillData->mAimDefaultRange);
		SkillUI.mTargeting.mSelectRangeRatio = StaticSkillData->mAimRatioRange;
		SkillUI.mTargeting.mHitShape = ToCombatSkillHitShape(StaticSkillData->mEffectPattern);
		SkillUI.mTargeting.mHitRange = StaticCast<float>(StaticSkillData->mEffectDefaultArea);
		SkillUI.mTargeting.mHitRangeRatio = StaticSkillData->mEffectRatioArea;
