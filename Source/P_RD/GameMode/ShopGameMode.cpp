...
void AShopGameMode::FillOwnedItems(FShopUI& ShopUIData) const
{
	...
	// 보유 직업은 실제 UnitIndex/슬롯을 복사하고, 미보유 직업은 조회 전용 카드다.
		...
		JobIndex < static_cast<int32>(EUnitJobType::PlayerJobCount); ++JobIndex)
	{
		...
		if (Owned != nullptr)
		{
			ShopUIData.mSkillTargetUnits.Add(*Owned);
		}
		else
		{
			FShopOwnedUnitUI Preview;
			Preview.mJobType = JobType;
			Preview.mIsOwned = false;
			Preview.mUnitIndex = INDEX_NONE;
			ShopUIData.mSkillTargetUnits.Add(Preview);
		}
	}
}
