...
namespace
{
	 ...
	 * GameMode가 화면 표시 상태로 바꿔주면 위젯은 준비/선택/잠김 상태를 그리는 일만 맡는다.
	 */
 ...
 *
 * @details
 * 선택 가능한 방인지 GameMode 기준으로 다시 검사한 뒤, 입장 버튼이 사용할 선택 좌표만 저장한다.
 *
 * 왜 UI 클릭만 믿지 않는가:
 ...
 *
 * 왜 선택과 입장을 나누는가:
 * 지도에서 노드를 눌러 미리 선택해보고, 입장 버튼으로 확정하는 사용자 경험을 만들기 위해서다.
...
/**
 * @brief 현재 런의 스테이지 정보를 월드맵 표시용 배열로 변환한다.
 *
 * @details
