...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTeleportMovementBarrierTests,
	"P_RD.SRPG.TeleportMovement.Barrier",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)

/**
 * @brief 연출 대기(배리어 보유) 중 상태와 완료 순서 검증
 *  1) 방송 시점에 논리 좌표는 이미 도착 타일, 이동 중 상태
 *  2) 배리어 보유 중: 완료 통지 없음, 도착 진입 통지 없음, 새 이동/텔레포트 거부
 *  3) 배리어 해제: 도착 진입 통지 -> 완료 통지, 이동 종료 상태
 */
bool FTeleportMovementBarrierTests::RunTest(const FString& Parameters)
{
	UWorld* World = GetAnyGameWorldForTeleportTests();
	if (World == nullptr)
	{
	...
	// 뷰 흉내: 배리어를 밖으로 빼서 잡아두고, 방송 시점의 상태를 기록
	TSharedPtr<FPresentationBarrier> HeldBarrier;
	int32 BroadcastCount = 0;
	FTileIndex BroadcastTileIndex;
	bool MovingAtBroadcast = false;
	FDelegateHandle TeleportHandle = Fixture.Unit->OnTeleport.AddLambda(
		[&](const FTileTransform& NextTileTransform, const FTransform&, TSharedPtr<FPresentationBarrier> Barrier)
		{
			++BroadcastCount;
			BroadcastTileIndex = NextTileTransform.mIndex;
			MovingAtBroadcast = Fixture.Movement->IsMoving();
			HeldBarrier = Barrier;
		});
	...
	/* 방송 시점 */
	TestTrue(TEXT("시작 성공"), Started);
	TestEqual(TEXT("OnTeleport 방송 1회"), BroadcastCount, 1);
	TestTrue(TEXT("방송된 목표 타일"), BroadcastTileIndex == FTileIndex(5, 5));
	TestTrue(TEXT("방송 시점에 이동 중 상태"), MovingAtBroadcast);
	TestTrue(TEXT("방송 시점에 논리 좌표는 이미 도착 타일"), Fixture.Unit->GetTileTransform().mIndex == FTileIndex(5, 5));

	/* 배리어 보유 중 */
	TestTrue(TEXT("배리어 보유 중"), HeldBarrier.IsValid());
	TestEqual(TEXT("보유 중 완료 통지 없음"), FinishCount, 0);
	TestEqual(TEXT("보유 중 도착 진입 통지 없음"), EndSensor->mBeginCount - EndBeginBase, 0);
	TestTrue(TEXT("보유 중 이동 중 상태"), Fixture.Movement->IsMoving());
	TestFalse(TEXT("보유 중 재텔레포트 거부"), Fixture.Movement->TeleportTo(FTileTransform(FTileIndex(6, 6), ETileActorDirection::Forward)));
	TestFalse(TEXT("보유 중 걷기 거부"), Fixture.Movement->MoveAlongPath({ FTileIndex(5, 5), FTileIndex(5, 6) }));

	/* 배리어 해제 */
	HeldBarrier.Reset();
	TestEqual(TEXT("해제 후 도착 진입 통지 1회"), EndSensor->mBeginCount - EndBeginBase, 1);
	TestEqual(TEXT("해제 후 완료 통지 1회"), FinishCount, 1);
	TestFalse(TEXT("해제 후 이동 종료 상태"), Fixture.Movement->IsMoving());
	TestTrue(TEXT("해제 후에도 도착 타일 유지"), Fixture.Unit->GetTileTransform().mIndex == FTileIndex(5, 5));

	Fixture.Unit->OnTeleport.Remove(TeleportHandle);
	return true;
}
