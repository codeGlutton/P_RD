/*****************************************************************//**
 * @file   PartyArtifactModelTests.cpp
 * @brief  UPartyArtifactModel 자동화 테스트
 * @details
...
#include "Pawn/Player/PartyArtifactTestsHelper.h"
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FPartyArtifactModelTests,
	"P_RD.Pawn.PartyArtifactModel",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)

bool FPartyArtifactModelTests::RunTest(const FString& Parameters)
{
	// 파티 모델 + 구성원 2명 생성
	UPartyArtifactModel* PartyArtifactModel = NewObject<UPartyArtifactModel>();
	...
	if (!TestNotNull(TEXT("파티 아티팩트 모델 생성"), PartyArtifactModel)
		|| !TestNotNull(TEXT("구성원 A 생성"), MemberA)
		|| !TestNotNull(TEXT("구성원 B 생성"), MemberB))
	{
		return false;
	}
	TArray<UPlayerUnitModel*> PartyMembers = { MemberA, MemberB };
	...
	TestTrue(TEXT("아티펙트 A 획득 성공"), PartyArtifactModel->AddArtifact(ArtifactA, PartyMembers));
	TestTrue(TEXT("아티펙트 B 획득 성공"), PartyArtifactModel->AddArtifact(ArtifactB, PartyMembers));
	...
	TestFalse(TEXT("nullptr 획득 실패"), PartyArtifactModel->AddArtifact(static_cast<UStaticArtifactData*>(nullptr), PartyMembers));
	TestEqual(TEXT("실패 후 파티 목록 그대로 2개"), PartyArtifactModel->GetPartyArtifacts().Num(), 2);

	// 제거: 파티 목록에서 빠지고 구성원 전원에서 해제돼야 함
	TestTrue(TEXT("아티펙트 A 제거 성공"), PartyArtifactModel->RemoveArtifact(ArtifactA, PartyMembers));
	...
	// 미보유 제거는 실패
	TestFalse(TEXT("미보유 A 재제거 실패"), PartyArtifactModel->RemoveArtifact(ArtifactA, PartyMembers));
