...
struct FTargetFixture
{
    ...
    FTargetFixture()
    {
        ...
        SetPolicy(EEnemyTargetPriority::Nearest);
    ...
    void SetPolicy(EEnemyTargetPriority Priority)
    {
        Data->mTargetPolicy.mPriority = Priority;
...
{
    FTargetFixture F;
    TestTrue(TEXT("Existing DAs default to nearest"), F.Data->mTargetPolicy.mPriority == EEnemyTargetPriority::Nearest);
    TestTrue(TEXT("Nearest"), F.Target() == FTileIndex(2, 2));
    F.SetPolicy(EEnemyTargetPriority::LowestHP);
    TestTrue(TEXT("DA policy reaches model"), F.Enemy->GetTargetPolicy().mPriority == EEnemyTargetPriority::LowestHP);
    TestTrue(TEXT("Lowest HP can beat a nearer target"), F.Target() == FTileIndex(4, 2));
    F.SetPolicy(EEnemyTargetPriority::LowestHPPercent);
    TestTrue(TEXT("HP ratio differs from absolute HP"), F.Target() == FTileIndex(6, 2));
    F.SetPolicy(EEnemyTargetPriority::HighestMaxHP);
    TestTrue(TEXT("Highest max HP"), F.Target() == FTileIndex(6, 2));
    F.SetPolicy(EEnemyTargetPriority::Farthest);
...
{
    FTargetFixture F;
    F.SetPolicy(EEnemyTargetPriority::Random);
...
bool FEnemyTargetChaseTest::RunTest(const FString&)
{
    ...
    F.Data->mTargetPolicy.mUnreachableBehavior = EEnemyUnreachableTargetBehavior::ChasePreferred;
    F.SetPolicy(EEnemyTargetPriority::LowestHP);
...
bool FEnemyTargetSkillOverrideTest::RunTest(const FString&)
{
    ...
    F.Skill->mEnemyTargetPolicy.mPriority = EEnemyTargetPriority::WithoutStatus;
    F.Skill->mEnemyTargetPolicy.mStatusTag = Status;
    TestTrue(TEXT("Disabled override inherits monster"), F.Target() == FTileIndex(2,2));
    F.Skill->mOverrideEnemyTargetPolicy = true;
    TestTrue(TEXT("Spread status to nearest unaffected target"), F.Target() == FTileIndex(4,2));
    F.Skill->mEnemyTargetPolicy.mPriority = EEnemyTargetPriority::WithStatus;
    TestTrue(TEXT("Status combo targets affected unit"), F.Target() == FTileIndex(2,2));
    F.Skill->mEnemyTargetPolicy.mStatusTag = FGameplayTag();
    ...
    F.Skill->mEnemyTargetPolicy.mPriority = EEnemyTargetPriority::LowestHP;
    F.Skill->mEnemyTargetPolicy.mUnreachableBehavior = EEnemyUnreachableTargetBehavior::ChasePreferred;
    F.Skill->mAimRange = 1;
...
{
    FTargetFixture F;
    F.SetPolicy(EEnemyTargetPriority::MostTargets);
...
bool FEnemyTargetCloneTest::RunTest(const FString&)
{
    ...
    for (auto Priority : {EEnemyTargetPriority::Random, EEnemyTargetPriority::LowestHP, EEnemyTargetPriority::MostTargets})
    {
        ...
        TestTrue(TEXT("Room duplication preserves DA policy"), Enemy != F.Enemy && Enemy->GetTargetPolicy().mPriority == Priority);
...
bool FEnemyRealDATest::RunTest(const FString&)
{
        ...
        TEXT("/Game/BP/DataAsset/Unit/EnemyUnit/Stage2/DA_SkeletonBirdUnit.DA_SkeletonBirdUnit")})
    {
        ...
        TestTrue(TEXT("Original asset is unchanged"), Original->mTargetPolicy.mPriority == EEnemyTargetPriority::Nearest);
    }
    return !HasAnyErrors();
}
#endif
