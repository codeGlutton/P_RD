...
/**
 * @brief 전투 모델 Mock (라운드 이벤트 등록/발동 검증용)
 * @details 서브시스템 초기화 없이 NewObject로만 만들어 씀. 타일맵은 테스트가 주입
 */
UCLASS()
class UMockGimmickCombatModel : public USRPGCombatModel
{
	GENERATED_BODY()
...
public:
	...
	void SetTileMap(UTileMapModel* TileMap)
	{
		mTileMapModel = TileMap;
	}

	// @brief 등록된 라운드 끝 이벤트 수 (중복 등록 검증용)
	int32 GetRoundEndEventCount() const
	{
		return mRoundEndEvents.Num();
	}

	// @brief 등록된 라운드 끝 이벤트를 전부 발동 (배리어 없음 = 동기 완주)
	void TriggerAllRoundEndEvents()
	{
		TriggerRoundEvents(nullptr, mRoundEndEvents, 0);
	}
};
...
/**
 * @brief 장판 기믹 Mock
 * @details 서브시스템 대신 테스트가 주입한 타일맵 사용. 스킬/라운드 수명은 테스트가 직접 세팅
 */
UCLASS()
class UMockPuddleGimmickModel : public UPuddleGimmickModel
{
	GENERATED_BODY()
...
public:
	...
	// @brief 남은 라운드 수 설정 (스폰 데이터 대체)
	void SetRemainingRoundCount(int32 Count)
	{
		mRemainingRoundCount = Count;
	}

	// @brief 남은 라운드 수 조회 (검증용)
	int32 GetRemainingRoundCount() const
	{
		return mRemainingRoundCount;
	}
