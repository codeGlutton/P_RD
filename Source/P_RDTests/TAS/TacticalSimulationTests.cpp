...
bool FTacticalSimulationTests::RunTest(const FString& Parameters)
{
    ...
    TestEqual(TEXT("초기 기본 방어력은 10.f이어야 합니다."), CompModel->GetAttributeBaseValue(UCombatTargetAttributeSet::GetDefenseAttribute()), 10.f);
    TestEqual(TEXT("초기 현재 방어력은 10.f이어야 합니다."), CompModel->GetAttributeCurrentValue(UCombatTargetAttributeSet::GetDefenseAttribute()), 10.f);
    ...
    // 복제 직후 수치 일치 여부 확인
    TestEqual(TEXT("[복제본] 기본 방어력은 10.f이어야 합니다."), ClonedCompModel->GetAttributeBaseValue(UCombatTargetAttributeSet::GetDefenseAttribute()), 10.f);
    TestEqual(TEXT("[복제본] 현재 방어력은 40.f이어야 합니다."), ClonedCompModel->GetAttributeCurrentValue(UCombatTargetAttributeSet::GetDefenseAttribute()), 40.f);
    ...
    TestEqual(TEXT("[복제본] 이펙트 제거 후, 방어력이 10.f로 복구되어야 합니다."), ClonedCompModel->GetAttributeCurrentValue(UCombatTargetAttributeSet::GetDefenseAttribute()), 10.f);
    
    // 원본은 여전히 방어력 버프가 유지되어 40이어야 함
    TestEqual(TEXT("[원본] 방어력이 40.f로 유지되어야 합니다."), CompModel->GetAttributeCurrentValue(UCombatTargetAttributeSet::GetDefenseAttribute()), 40.f);
