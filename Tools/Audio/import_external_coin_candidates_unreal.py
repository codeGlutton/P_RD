...
def main() -> None:
    source_wavs = sorted(SOURCE_DIR.glob("S_CoinGain_Ext_*.wav"))
    if len(source_wavs) != 10:
        raise RuntimeError(f"Expected 10 external coin WAVs, found {len(source_wavs)} in {SOURCE_DIR}")
    ...
    unreal.log(f"Imported {len(tasks)} external coin candidate SoundWaves to {DESTINATION_PATH}")
