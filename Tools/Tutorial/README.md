...
함정 종류는 `StaticGimmickSpawnData`의 PrimaryAssetId로 구별한다. 화면 안의 실제 함정 액터를 강조하고 해당 DA의 `DisplayName`, `Description`을 표시한다. 새 함정도 이 두 항목을 채우면 같은 흐름으로 안내된다. 빈 설명은 안내를 생략한다.

플레이어가 행동을 고를 수 있는 차례에 표시하며 기본 튜토리얼, 정보창, 설정, 전투 연출, 이동·스킬 선택이 끝나길 기다린다. 여러 미확인 종류가 있으면 한 번에 하나씩 표시한다. 화면 밖이나 사라진 함정을 가리키는 안내는 닫고 미확인 상태를 유지한다.

`확인`을 눌렀을 때만 프로필의 `SeenTrapHints`를 저장한다. 런 포기·새로하기·이어하기에도 유지되며 기본 튜토리얼 완료 여부와 독립적이다. 프로필 전체 초기화 시에만 함께 초기화한다. 안내 중에는 확인 버튼만 입력을 받는다.

기존 8종의 빈 설명은 `seed_trap_descriptions.py`로 작성했다. 이 스크립트는 기존 문구를 덮어쓰지 않는다. 현재 폭발 통·토템도 진입 트리거이며, 장판은 진입 시와 유닛이 올라선 라운드 종료 시 발동한다. 로직을 바꾸면 DA 설명도 함께 수정한다.
