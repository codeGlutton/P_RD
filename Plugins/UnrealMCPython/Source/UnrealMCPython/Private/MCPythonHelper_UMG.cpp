...
namespace
{
    ...
    {
        const TMap<FString, FString> TypeMap = {
    ...
    static void EnsureWidgetBlueprintGuid(UWidgetBlueprint* WidgetBP, const UWidget* Widget)
    {
        if (!WidgetBP || !Widget)
        {
            return;
        ...
        const FGuid* ExistingGuid = WidgetBP->WidgetVariableNameToGuidMap.Find(WidgetName);
        if (ExistingGuid == nullptr || !ExistingGuid->IsValid())
        {
            WidgetBP->Modify();
    ...
    static void RemoveWidgetBlueprintGuid(UWidgetBlueprint* WidgetBP, const UWidget* Widget)
    {
        if (!WidgetBP || !Widget)
        {
            return;
...
FString UMCPythonHelper::UmgAddWidget(UBlueprint* WidgetBP, const FString& WidgetType, const FString& WidgetName, const FString& ParentName)
{
    ...
    // 위젯을 '변수'로 만들지 않는다(non-variable). 절차생성 WBP의 위젯을 전부 변수+GUID로 등록하면
    // 생성클래스/WidgetVariableNameToGuidMap이 비대해져(수십~백 개) Android unversioned 쿡에서
    // export 테이블 참조가 깨진다(ObjectSerializationError: Bad export index → 로드시 RequestExit).
    // C++는 WidgetTree->FindWidget(이름)으로 찾으므로 변수/GUID가 필요 없다. BindWidgetOptional도 폴백 처리됨.
    NewWidget->bIsVariable = false;
...
FString UMCPythonHelper::UmgRemoveWidget(UBlueprint* WidgetBP, const FString& WidgetName)
{
    ...
    WT->Modify();
    RemoveWidgetBlueprintGuid(WB, Widget);
...
FString UMCPythonHelper::UmgSetWidgetIsVariable(UBlueprint* WidgetBP, const FString& WidgetName, bool bIsVariable)
{
    ...
    EnsureWidgetBlueprintGuid(WB, Widget);
    ...
    float OffsetX, float OffsetY, float SizeX, float SizeY)
{
    ...
    // Alignment(0,0): Offset(X,Y)를 위젯의 좌상단으로, Size(X,Y)를 크기로 그대로 해석한다.
    // (0.5,0.5)면 점앵커+사이즈 슬롯에서 위젯이 자기 크기의 절반만큼 좌상단으로 밀려(=Offset이 '중심'이 됨)
    // 디자인 JSON의 top-left screenRect와 어긋나 HUD 전체가 좌상단으로 시프트된다(특히 큰 버튼 프레임에서 명백).
    // 호출부(set_canvas_layout)는 Offset=top-left 의도로 호출하므로 (0,0)이 맞다.
    Data.Alignment = FVector2D(0.0f, 0.0f);
    CPS->SetLayout(Data);
...
FString UMCPythonHelper::UmgWrapWidget(UBlueprint* WidgetBP, const FString& WidgetName, const FString& WrapperType, const FString& WrapperName)
{
    ...
    EnsureWidgetBlueprintGuid(WB, Wrapper);
    EnsureWidgetBlueprintGuid(WB, Widget);
...
FString UMCPythonHelper::UmgReplaceWidget(UBlueprint* WidgetBP, const FString& WidgetName, const FString& NewType, const FString& NewName)
{
    ...
    EnsureWidgetBlueprintGuid(WB, NewWidget);
    RemoveWidgetBlueprintGuid(WB, Widget);
...
FString UMCPythonHelper::UmgBindWidgetEvent(UBlueprint* WidgetBP, const FString& WidgetName, const FString& EventName)
{
    ...
    EnsureWidgetBlueprintGuid(WB, W);
